...
const instanceChangedRegexes = {
  en: netregexes/* default.gameLog */.ZP.gameLog({
    code: '0039',
    line: 'You are now in the instanced area.*?'
  }),
  de: netregexes/* default.gameLog */.ZP.gameLog({
    code: '0039',
    line: 'Du bist nun in dem instanziierten Areal.*?'
  }),
  fr: netregexes/* default.gameLog */.ZP.gameLog({
    code: '0039',
    line: 'Vous êtes maintenant dans la zone instanciée.*?'
  }),
  ja: netregexes/* default.gameLog */.ZP.gameLog({
    code: '0039',
    line: 'インスタンスエリア.*?'
  }),
  cn: netregexes/* default.gameLog */.ZP.gameLog({
    code: '0039',
    line: '当前所在副本区为.*?'
  }),
...
const PlayPopSound = (monster, options) => {
  if (options.TTS) {
...
const PlayPullSound = (monster, options) => {
  // Only play sounds for S ranks and above.
  if (monster.rank !== 'S' && monster.rank !== 'SS+' && monster.rank !== 'SS-') return;
...
class Radar {
  ...
  AddMonster(log, hunt, matches) {
    ...
    if (targetMob) {
      ...
      if (newDistance < oldDistance) {
        ...
        targetMob.posZ = parseFloat(matches.z ?? '0'); // Update DOM

        this.UpdateMonsterDom(targetMob); // Play sound only if its far enough

        if (oldPos.distance(newPos) >= kMinDistanceBeforeSound) PlayPopSound(targetMob, options);
      }
    } else {
      // Add DOM
      ...
      img.setAttribute('src', arrow_namespaceObject);
      ...
      if (`${m.rank}` == 'A' || `${m.rank}` == 'B' || spotzone == '갈레말드' || spotzone == '라비린토스' || spotzone == '울티마 툴레' || spotzone == '아므 아랭' || spotzone == '일 메그' ||
          spotzone == '얀샤' || spotzone == '아짐 대초원' || spotzone == '커르다스 서부고지' || spotzone == '저지 라노시아' || spotzone == '고지 라노시아' || spotzone == '검은장막 숲 남부삼림' ||
          spotzone == '중부 다날란') {
        $.ajax({
        type: "GET",
        ...
        data: {
          ...
          "자리 위치": document.getElementById('zone').textContent,
          "자리 x좌표": `${mapX}`,
          "자리 y좌표": `${mapY}`,
  ...
  UpdateMonsterPuller(monster, puller) {
    if (monster.puller !== undefined || monster.alreadyPulled) return;
  ...
  UpdateMonsterDom(monster) {
    ...
    if (node && node instanceof HTMLElement) {
      ...
      if (monster.puller !== undefined) node.innerHTML += '&nbsp;&nbsp;' + monster.puller; // Z position is relative to the map so it's omitted.
  ...
  RemoveMonster(mobKey) {
    ...
    const mapX = posToMap(monster.pos.x).toFixed(1);
    const mapY = posToMap(monster.pos.y).toFixed(1);
    if (!monster) return;
    //console.log(`Killed: ${monster.name}`);
    //console.log(`Killed: ${monster.world}`);
  ...
  OnNetLog(e) {
    ...
    if (type === '00') {
      if (this.regexes.instanceChanged.test(log)) {
        ...
        if (instancecut === '') {var instancenumber = 1;}
        if (instancecut === '') {var instancenumber = 2;}
        if (instancecut === '') {var instancenumber = 3;}
        document.getElementById('instancenumber').textContent = instancenumber;
      }
      if (this.regexes.worldChanged.test(log)) {
        ...
        //console.log(document.getElementById('myFiel').value);
      }
    ...
    if (type === '01') {
      ...
      if (name == '갈레말드' || name == '라비린토스' || name == '울티마 툴레' || name == '아므 아랭' || name == '일 메그' || name == '얀샤' || name == '아짐 대초원' || 
          name == '커르다스 서부고지' || name == '저지 라노시아' || name == '고지 라노시아' || name == '검은장막 숲 남부삼림' || name == '중부 다날란') {
...
if (document.getElementById('server').textContent == '') {document.getElementById('servererr').textContent = '미확인! 서버 텔레포를 1회 실행해 주세요.'}
          ...
          heading_2.innerHTML = "지역";
          let heading_3 = document.createElement('th');
          heading_3.innerHTML = "예정시간";
          let heading_4 = document.createElement('th');
          heading_4.innerHTML = "진행률";
          ...
          let row_$i_data_1 = document.createElement('td');
          row_$i_data_1.innerHTML = data.items[i].huntname;
          let row_$i_data_2 = document.createElement('td');
          row_$i_data_2.innerHTML = data.items[i].huntzone;
          let row_$i_data_3 = document.createElement('td');
          row_$i_data_3.innerHTML = data.items[i].hunttime;
          let row_$i_data_4 = document.createElement('td');
          row_$i_data_4.innerHTML = data.items[i].huntprog;

          row_$i.appendChild(row_$i_data_1);
          row_$i.appendChild(row_$i_data_2);
          row_$i.appendChild(row_$i_data_3);
          row_$i.appendChild(row_$i_data_4);
          tbody.appendChild(row_$i);
          }
          document.getelementbyid("bodydata");
            //성공시 들어갈 코드
          }).fail(function(data){
            //실패시 들어갈 코드
          });
